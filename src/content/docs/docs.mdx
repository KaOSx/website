...
KaOS provides you with an easy to use option to give very complete info about all of your system. Open Octopi and go to the _Tools_ menu, there click on _Sysinfo_, or you can use your keyboard in Octopi with shortcut:
...
1. How did you create the Live Media? Make sure to follow the tutorials to properly create installation media for KaOS. See [USB](/install/usb_flash/) and [DVD](/install/dvd/) creation guides.
...
3. Are you using a GPT partition table on a BIOS system? If so, follow the instruction to set up properly since the installer cannot create such a setup yet. See [Create GPT on BIOS](/install/bios_gpt)
